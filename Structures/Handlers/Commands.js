...
module.exports = async (client, PG, Ascii) => {
    ...
    (await PG(`${process.cwd()}/Commands/**/*.js`)).map(async (file) => {
        ...
        if (!command.type && !command.description)
            return Table.addRow( command.name, '🔹 Successful', 'Missing a cooldown.' );
    ...
    client.on('ready', async () => {
        if (process.env.NODE_ENV.trim() === 'production') {
            log.log(`EnlX > Commands | Running Production Mode`);
            log.info(`EnlX > Commands | ${client.commands.size} commands loaded.`);
            client.guilds.cache.forEach((g) => {
                g.commands.set(CommandsArray).then(async (command) => {
                    ...
                    const fullPermissions = command.reduce((accumulator, r) => {
                        ...
                        const permissions = roles.reduce((a, r) => {return [...a, { id: r.id, type: 'ROLE', permission: true },];}, []);
            ...
            log.log(`EnlX > Commands | Running Development Mode`);
            log.info(`EnlX > Commands | ${client.commands.size} commands loaded.`);
